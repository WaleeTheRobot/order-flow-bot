...
namespace NinjaTrader.Custom.AddOns.OrderFlowBot.Strategies
{
    ...
    public class PivotFader : StrategyBase
    {
        ...
        public PivotFader(OrderFlowBotState orderFlowBotState, OrderFlowBotDataBars dataBars, string name, TechnicalLevels technicalLevels)
        ...
        {
            if (dataBars.Bars.Count < _count || technicalLevels.Pivots.Count < _pivotCount)
            {
                return;
            }
            ...
            _pivotPoints = technicalLevels.Pivots.Skip(Math.Max(0, technicalLevels.Pivots.Count - _pivotCount)).Take(_pivotCount).ToList();
            // This is the current not complete pivot. This doesn't complete until after the next bar after it closes.
            _currentPivot = technicalLevels.CurrentPivot;
        ...
        private bool IsLookingForHigh()
        {
            ...
            return technicalLevels.IsLookingForHigh;
        }
        ...
        private bool IsValidPreviousLevel(bool checkLong = true)
        {
            if (checkLong)
            {
                ...
                foreach (var pivot in _pivotPoints)
                {
                    if (!pivot.IsHigh)
                    {
                        ...
                        //technicalLevels.currentLow = _currentPivot.Price;
                        //technicalLevels.previousLow = pivot.Price;
            ...
            foreach (var pivot in _pivotPoints)
            {
                if (pivot.IsHigh)
                {
                    ...
                    // Example set for debugging
                    //technicalLevels.currentHigh = _currentPivot.Price;
                    //technicalLevels.previousHigh = pivot.Price;
